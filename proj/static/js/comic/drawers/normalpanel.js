class NormalPanel {
    ...
    drawAnimation() {
        let marginLeft = this.getMarginLeft();
        for(var i = 0; i < this.loader.getAnimations.length; i++) {
    ...
    drawPattern() {
        ...
        backGround.graphics.clear().beginFill(pat.color).drawRect(0, 0, 980, 960);
        patternContainer.addChild(backGround);

        for(var i = 0; i < this.loader.patterns.length; i++){
        ...
        fillLeft.graphics.clear().beginFill(fillColor).drawRect(0, 0, marginLeft / 2, 960);
        this.stage.addChild(fillLeft);

        let fillRight = new createjs.Shape();
        fillRight.graphics.clear().beginFill(fillColor).drawRect((marginLeft / 2 + this.loader.getPanelImg.image.width), 0, marginLeft / 2 , 960);
