class PatternMaker {
	...
	load() {
		...
		while (coldArr.length < 3) {
			let coldImg = '/static/img/comic/pattern/cold/' + (this.getRandomInt(15) + 1) + '.png';
		...
		while (warmArr.length < 3) {
			let warmImg = '/static/img/comic/pattern/warm/' + (this.getRandomInt(15) + 1) + '.png';
			if (warmArr.indexOf(warmImg) === -1) {
				warmArr.push(warmImg);
			}
		}
		warmArr.forEach(w => this.queue.loadFile({ id: 'warm', src: w }));
		coldArr.forEach(c => this.queue.loadFile({ id: 'cold', src: c }));
	}
	...
	setImgsToDiv() {
		for (let i = 0; i < this.warmImgsArr.length; i++) {
			console.log(i);
			let div = document.getElementById('pattern-warm-' + i);
			div.style.backgroundImage = 'url(' + this.warmImgsArr[i].image.src + ')';
			div.addEventListener('click', evt => this.onClickWarm(evt));
		}
		for (let i = 0; i < this.coldImgsArr.length; i++) {
			let div = document.getElementById('pattern-cold-' + i);
	...
	onClickWarm(evt) {
		let id = evt.target.id.slice(13, 14);
	...
	decreaseLayer() {
		if (this.activePattern) {
			let newindex = this.patternContainer.getChildIndex(this.activePattern) - 1;
	...
	save() {
		...
		let siteUrl = location.href.slice(0, location.href.indexOf('pattern/'));
		let mood = location.href.slice(location.href.lastIndexOf('/pattern/') + 9, location.href.length - 1);
		if (mood === 'happy') {
	...
	makeCookie() {
		...
		let pattern = { color: this.color };
		pattern.pattern = [];
		for (let i = 0; i < this.patternContainer.numChildren; i++) {
			let child = this.patternContainer.getChildAt(i);
